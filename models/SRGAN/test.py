...
def get_model():
    ...
    checkpoint = torch.load("models/SRGAN/srgan.pth.tar", map_location=lambda storage, loc: storage)
...
def main() -> None:
    ...
    model.load_state_dict(checkpoint["state_dict"])
    print(
        f"Load SRGAN model weights `{os.path.abspath(config.model_path)}` successfully."
    )
    ...
    file_names_hr = natsorted(os.listdir(config.hr_dir))
    ...
    for index in range(total_files):
        ...
        hr_image_path = os.path.join(config.hr_dir, file_names_hr[index])

        print(hr_image_path)
        print(f"Processing `{os.path.abspath(lr_image_path)}`...")
        # Read LR image and HR image
        lr_image = (
            cv2.imread(lr_image_path, cv2.IMREAD_UNCHANGED).astype(np.float32) / 255.0
        )
        hr_image = (
            cv2.imread(hr_image_path, cv2.IMREAD_UNCHANGED).astype(np.float32) / 255.0
        )
        ...
        lr_tensor = imgproc.image2tensor(
            lr_image, range_norm=False, half=False
        ).unsqueeze_(0)
        hr_y_tensor = imgproc.image2tensor(
            hr_image, range_norm=False, half=False
        ).unsqueeze_(0)
        ...
        sr_y_tensor = imgproc.image2tensor(
            sr_y_image, range_norm=False, half=False
        ).unsqueeze_(0)

        # Copy to CUDA
        sr_y_tensor = sr_y_tensor.to(
            device=config.device, memory_format=torch.channels_last, non_blocking=True
        )

        total_psnr += 10.0 * torch.log10_(
            1.0 / torch.mean((sr_y_tensor - hr_y_tensor) ** 2)
        )
