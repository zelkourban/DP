...
def get_model():
    model = EDSR(config.upscale_factor)
    checkpoint = torch.load(
        "models/EDSR/edsr_realx4.pth.tar", map_location=lambda storage, loc: storage
    )
...
def main() -> None:
    ...
    model.load_state_dict(checkpoint["state_dict"])
    print(
        f"Load EDSR model weights `{os.path.abspath(config.model_path)}` successfully."
    )
    ...
    for index in range(total_files):
        ...
        lr_tensor = (
            imgproc.image2tensor(lr_image, range_norm=False, half=False)
            .to(config.device)
            .unsqueeze_(0)
        )
        hr_y_tensor = (
            imgproc.image2tensor(hr_image, range_norm=False, half=False)
            .to(config.device)
            .unsqueeze_(0)
        )
        ...
        sr_y_tensor = (
            imgproc.image2tensor(sr_y_image, range_norm=False, half=False)
            .to(config.device)
            .unsqueeze_(0)
        )

        total_psnr += 10.0 * torch.log10(
