...
def get_model():
    ...
    checkpoint = torch.load(
        "models/SRCNN/srcnn_x4.pth.tar", map_location=lambda storage, loc: storage
    )
...
def main() -> None:
    ...
    model.load_state_dict(checkpoint["state_dict"])
    print(
        f"Load SRCNN model weights `{os.path.abspath(config.model_path)}` successfully."
    )
    ...
    for index in range(total_files):
        ...
        lr_image = lr_image[
            : lr_image_height - lr_image_height_remainder,
            : lr_image_width - lr_image_width_remainder,
            ...,
        ]
        ...
        hr_image = hr_image[
            : hr_image_height - hr_image_height_remainder,
        ...
        lr_y_tensor = (
            imgproc.image2tensor(lr_y_image, range_norm=False, half=True)
            .to(config.device)
            .unsqueeze_(0)
        )
        hr_y_tensor = (
            imgproc.image2tensor(hr_y_image, range_norm=False, half=True)
            .to(config.device)
            .unsqueeze_(0)
        )
