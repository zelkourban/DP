...
def get_model():
    ...
    checkpoint = torch.load(
        "models/RealESRGAN/RealESRGAN_x4.pth.tar", map_location=lambda storage, loc: storage
    )
...
def main() -> None:
    ...
    model.load_state_dict(checkpoint["state_dict"])
    print(
        f"Load ESRGAN model weights `{os.path.abspath(config.model_path)}` successfully."
    )
    ...
    for index in range(total_files):
        ...
        hr_y_tensor = imgproc.image2tensor(
            hr_y_image, range_norm=False, half=True
        ).unsqueeze_(0)
        ...
        sr_y_tensor = imgproc.image2tensor(
            sr_y_image, range_norm=False, half=True
        ).unsqueeze_(0)
